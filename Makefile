...
WITH_MPP ?= 1
WITH_ALSA ?= 1
BUILD_DIR ?= build/mpp$(WITH_MPP)-alsa$(WITH_ALSA)
BIN_DIR ?= bin
...
CPPFLAGS += -Iinclude -DIPC_WITH_MPP=$(WITH_MPP) -DIPC_WITH_ALSA=$(WITH_ALSA)
...
host-capture-test:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=0 BUILD_DIR=build/host BIN_DIR=bin/host capture-mock
	python3 tests/test_capture.py ./bin/host/ipc_camera_mock

host-capture-sanitize:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=0 BUILD_DIR=build/capture-asan BIN_DIR=bin/capture-asan CFLAGS='-std=c11 -O1 -g -Wall -Wextra -Wpedantic -Werror -pthread -fsanitize=address,undefined -fno-omit-frame-pointer -fno-pie' LDFLAGS='-fsanitize=address,undefined -no-pie' capture-mock
...
host-check:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=0 BUILD_DIR=build/host BIN_DIR=bin/host all
...
host-queue-test:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=0 BUILD_DIR=build/host BIN_DIR=bin/host queue-test
	./bin/host/test_frame_queue

# 可选内存检查，仅用于支持 sanitizer 的主机编译器，不部署到 RK3568。
host-queue-sanitize:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=0 BUILD_DIR=build/queue-asan BIN_DIR=bin/queue-asan CFLAGS='-std=c11 -O1 -g -Wall -Wextra -Wpedantic -Werror -pthread -fsanitize=address,undefined -fno-omit-frame-pointer' LDFLAGS='-fsanitize=address,undefined' queue-test
...
host-encoder-test:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=1 MPP_INCLUDE=tests/mpp_headers MPP_LIBS= BUILD_DIR=build/encoder-mock BIN_DIR=bin/encoder-mock CFLAGS='-std=c11 -O2 -g -Wall -Wextra -Wpedantic -Werror -pthread' encoder-mock
...
host-encoder-sanitize:
	$(MAKE) CC=cc SYSROOT= WITH_ALSA=0 WITH_MPP=1 MPP_INCLUDE=tests/mpp_headers MPP_LIBS= BUILD_DIR=build/encoder-asan BIN_DIR=bin/encoder-asan CFLAGS='-std=c11 -O1 -g -Wall -Wextra -Wpedantic -Werror -pthread -fsanitize=address,undefined -fno-omit-frame-pointer -fno-pie' LDFLAGS='-fsanitize=address,undefined -no-pie' encoder-mock
...
host-audio-test:
	$(MAKE) CC=cc SYSROOT= WITH_MPP=0 WITH_ALSA=1 ALSA_INCLUDE=tests/alsa_headers ALSA_LIBS= BUILD_DIR=build/audio-mock BIN_DIR=bin/audio-mock CFLAGS='-std=c11 -O2 -g -Wall -Wextra -Wpedantic -Werror -pthread' audio-mock
	python3 tests/test_audio.py ./bin/audio-mock/ipc_camera_audio_mock
	./bin/audio-mock/test_audio_api configs/ipc.conf

host-audio-sanitize:
	$(MAKE) CC=cc SYSROOT= WITH_MPP=0 WITH_ALSA=1 ALSA_INCLUDE=tests/alsa_headers ALSA_LIBS= BUILD_DIR=build/audio-asan BIN_DIR=bin/audio-asan CFLAGS='-std=c11 -O1 -g -Wall -Wextra -Wpedantic -Werror -pthread -fsanitize=address,undefined -fno-omit-frame-pointer -fno-pie' LDFLAGS='-fsanitize=address,undefined -no-pie' audio-mock
